	...
	, mIHostProcessorDelegateV3(NULL)
{
	if ( pUnknown )
	{
		...
		else if (mIHostProcessorDelegateV2)
		{
...
// METHOD:	GetAudio
// ******************************************************************************************
AAX_Result AAX_VHostProcessorDelegate::GetAudio ( const float * const inAudioIns [], int32_t inAudioInCount, int64_t iLocation, int32_t * ioNumSamples )
{
	if ( mIHostProcessorDelegate )
		return mIHostProcessorDelegate->GetAudio( inAudioIns, inAudioInCount, iLocation, ioNumSamples );
