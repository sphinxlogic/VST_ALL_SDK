...
AAX_Result	AAX_CEffectParameters::NotificationReceived(AAX_CTypeID iNotificationType, const void * iNotificationData, uint32_t /*iNotificationDataSize*/)
{
	if (AAX_eNotificationEvent_ASPreviewState == iNotificationType)
    {
		AAX_IParameter* parameter = mParameterManager.GetParameterByID(cPreviewID);
		if (parameter != 0)
		{
			int32_t previewState = *reinterpret_cast<const int32_t*>(iNotificationData);
...
AAX_Result AAX_CEffectParameters::ResetFieldData (AAX_CFieldIndex /*iFieldIndex*/, void* oData, uint32_t iDataSize) const
{
...
{
	*oControlIndex = mParameterManager.ControlIndexFromID( iParameterID );
	if (*oControlIndex < 0)
		return AAX_ERROR_INVALID_PARAMETER_ID;
...
//----------------------------------------------------------------------------------------------
AAX_Result	AAX_CEffectParameters::GetCustomData( AAX_CTypeID /*iDataBlockID*/, uint32_t /*iDataSize*/, void* /*oData*/, uint32_t* oDataWritten) const
{
	*oDataWritten = 0;
	return AAX_SUCCESS;
}
...
//----------------------------------------------------------------------------------------------
AAX_Result	AAX_CEffectParameters::SetCustomData( AAX_CTypeID /*iDataBlockID*/, uint32_t /*iDataSize*/, const void* /*iData*/ )
{
	return AAX_SUCCESS;
}
...
//----------------------------------------------------------------------------------------------
AAX_Result AAX_CEffectParameters::UpdateMIDINodes( AAX_CFieldIndex /*iFieldIndex*/, AAX_CMidiPacket& /*iPacket*/  )
{
	return AAX_SUCCESS;	
}
