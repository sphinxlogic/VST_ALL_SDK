...
protected:
	...
	CACFClassFactory(const CACFClassFactory&) {}

	virtual ~CACFClassFactory();

	// ACFUnknown override
	ACFMETHOD(InternalQueryInterface)(const acfIID & riid, void **ppv);

public:
	// IACFClassFactory methods 
	ACFMETHOD(CreateInstance)(IACFUnknown * pUnkHost, IACFUnknown * pUnkOuter, const acfIID & riid, void ** ppv); 
