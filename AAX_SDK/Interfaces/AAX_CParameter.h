...
public: // AAX_IParameterValue implementation
	...
	virtual AAX_IParameterValue*	Clone() const { return new AAX_CParameterValue<T>(*this); }
	virtual AAX_CParamID			Identifier() const { return mIdentifier; }
	...
	virtual bool					GetValueAsInt32(int32_t* value) const;
	virtual bool					GetValueAsFloat(float* value) const;
	virtual bool					GetValueAsDouble(double* value) const;
	virtual bool					GetValueAsString(AAX_IString* value) const;
	//@} Typed accessors
...
private:
	AAX_CParameterValue& operator=(const AAX_CParameterValue&); // Not defined
};
...
public:
	...
	virtual AAX_IParameterValue*	CloneValue() const;										//AAX_IParameter
	
	/** @name Identification methods
	 *
	 */
	//@{
	AAX_CParamID		Identifier() const;													//AAX_IParameter					
	void				SetName(const AAX_CString& name);									//AAX_IParameter		
	const AAX_CString&	Name() const;														//AAX_IParameter
	void				AddShortenedName(const AAX_CString& name);							//AAX_IParameter
	const AAX_CString&	ShortenedName(int32_t iNumCharacters) const;						//AAX_IParameter
	void				ClearShortenedNames();												//AAX_IParameter
	 ...
	 */
	//@{
	void				SetNormalizedDefaultValue(double normalizedDefault);				//AAX_IParameter
	double				GetNormalizedDefaultValue() const;									//AAX_IParameter		
	void				SetToDefaultValue();												//AAX_IParameter				
	void				SetNormalizedValue(double normalizedNewValue);						//AAX_IParameter		
	double				GetNormalizedValue()	const;										//AAX_IParameter		
	void				SetNumberOfSteps(uint32_t numSteps);								//AAX_IParameter			
	uint32_t			GetNumberOfSteps() const;											//AAX_IParameter	
	uint32_t			GetStepValue() const;												//AAX_IParameter	
	void				SetStepValue(uint32_t iStep);										//AAX_IParameter	
	void				SetType(AAX_EParameterType iControlType);							//AAX_IParameter
	AAX_EParameterType	GetType() const;													//AAX_IParameter		
	 ...
	 */
	//@{
	bool				GetValueString( AAX_CString*	valueString) const;																		//AAX_IParameter
	bool				GetValueString(int32_t iMaxNumChars, AAX_CString*	valueString) const;													//AAX_IParameter
	bool				GetNormalizedValueFromString(const AAX_CString&	valueString, double *normalizedValue) const;							//AAX_IParameter
	bool				GetStringFromNormalizedValue(double normalizedValue, AAX_CString&	valueString) const;									//AAX_IParameter
	bool				GetStringFromNormalizedValue(double normalizedValue, int32_t iMaxNumChars, AAX_CString&	valueString) const;			//AAX_IParameter
	bool				SetValueFromString(const AAX_CString&	newValueString);																//AAX_IParameter
	//@} Display methods
	 ...
	 */
	//@{
	void				SetAutomationDelegate ( AAX_IAutomationDelegate * iAutomationDelegate );	//AAX_IParameter
	bool				Automatable() const;														//AAX_IParameter
	void				Touch();																	//AAX_IParameter
	void				Release();																	//AAX_IParameter
	//@} Automation methods
	 ...
	 */
	//@{
	bool				GetValueAsBool(bool* value) const;									//AAX_IParameter
	bool				GetValueAsInt32(int32_t* value) const;								//AAX_IParameter
	bool				GetValueAsFloat(float* value) const;								//AAX_IParameter
	bool				GetValueAsDouble(double* value) const;								//AAX_IParameter
	bool				GetValueAsString(AAX_IString* value) const;							//AAX_IParameter
	bool				SetValueWithBool(bool value);										//AAX_IParameter
	bool				SetValueWithInt32(int32_t value);									//AAX_IParameter
	bool				SetValueWithFloat(float value);										//AAX_IParameter
	bool				SetValueWithDouble(double value);									//AAX_IParameter
	bool				SetValueWithString(const AAX_IString& value);						//AAX_IParameter
	//@} Typed accessors
...
public: 
	 ...
	 */
	//@{
	void	UpdateNormalizedValue(double newNormalizedValue);								//AAX_IParameter
...
private:
	...
	AAX_CParameter();
	AAX_CParameter(const AAX_CParameter& other);
	AAX_CParameter&	operator= (const AAX_CParameter& other);
};
...
, mOrientation( AAX_eParameterOrientation_BottomMinTopMax )
...
{
	uint32_t stringLength = name.Length();
	mShortenedNames[stringLength] = name;	//Does a string copy into the map.
}

template <typename T>
const AAX_CString&	AAX_CParameter<T>::ShortenedName(int32_t iNumCharacters) const
{
	//More characters than the full name of no specific shortened names.
	if ((iNumCharacters >= int32_t(mName.Length())) || (mShortenedNames.empty()) || (0 > iNumCharacters))
		return mName;
			
	std::map<uint32_t, AAX_CString>::const_iterator iter = mShortenedNames.upper_bound(static_cast<uint32_t>(iNumCharacters));
	
	//If the iterator is already pointing to shortest name, return that.
	if (iter == mShortenedNames.begin())	
		return iter->second;
	...
	if (mNumSteps == 0) // If no explicit number of steps has been set...
    {
        switch (mControlType)
        {
            ...
            case AAX_eParameterType_Continuous:
            {
                ...
                SetNumberOfSteps (eParameterDefaultNumStepsContinuous);
                break;
            }
            default:
            {
...
uint32_t	AAX_CParameter<T>::GetStepValue() const
{
	...
	double	value = this->GetNormalizedValue ();
	double	valuePerStep = 1.0 / ( numSteps - 1.0 );
	double	curStep = ( value / valuePerStep ) + 0.5;
	if ( curStep < 0.0 )
		curStep = 0.0;
...
{
	return mDisplayDelegate->ValueToString(GetValue(), valueString);
}
...
template<typename T>
void	AAX_CParameter<T>::SetTaperDelegate(AAX_ITaperDelegateBase& taperDelegate,bool iPreserveValue)
{
	double	normalizeValue = this->GetNormalizedValue ();

	AAX_ITaperDelegate<T>* oldDelegate = mTaperDelegate;
	mTaperDelegate = ((AAX_ITaperDelegate<T> &) taperDelegate).Clone();
	delete oldDelegate;
...
void	AAX_CParameter<T>::SetDisplayDelegate(AAX_IDisplayDelegateBase& displayDelegate)
{
	...
	mDisplayDelegate = ((AAX_IDisplayDelegate<T> &)displayDelegate).Clone();
	delete oldDelegate;
...
{ 	
	...
	if (mAutomationDelegate)
		mAutomationDelegate->PostTouchRequest( Identifier() ); 
}				
...
{ 	
	...
	if (mAutomationDelegate)
		mAutomationDelegate->PostReleaseRequest( Identifier() );
}					
