...
public:
	 ...
	 *
	 *	Called from the host when major changes take place such as instantiation, insert position, track name.
	 *  Most of these notifications are not sent from the host at this point, but we're putting this function in as
	 *  a placeholder for plug-ins to be notified when events happen.
	 *
	 ...
	 *		Type of notification being received
	 ...
	 */
	virtual	AAX_Result			NotificationReceived( /* AAX_ENotificationEvent */ AAX_CTypeID iNotificationType, const void * iNotificationData, uint32_t	iNotificationDataSize) = 0;
	 ...
	 *
	 *	\warning The curve data for a plug-in may not be refreshed every time the curve changes. See
	 *	\ref PTSW-195316 for more information.
	 *
	 *	\compatibility S6 currently polls this method to update a plug-in's EQ or dynamics curves
	 ...
	 *	curves displayed on S6. (GWSW-7314)
	 *
	 *	\param[in] iCurveType
...
/** @brief	Supplemental interface for an %AAX Plug-in's data model
	...
	%AAX v2 features may call into these methods. See \ref CommonInterface_DataModel.
...
/** @brief	Supplemental interface for an %AAX Plug-in's data model
	...
	This is a supplemental interface for an instance of a plug-in's data model. This
	interface gets exposed to the host application. Host applications that support
	%AAX v3 features may call into these methods. See \ref CommonInterface_DataModel.
...
/** @name EUCON Methods
	 ...
	 *
	 *	\return This method should return AAX_ERROR_UNIMPLEMENTED if the plug-in does not implement it.
	 *
	 */
