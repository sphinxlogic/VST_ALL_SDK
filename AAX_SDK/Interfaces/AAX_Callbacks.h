...
enum AAX_CProcPtrID
{
	kAAX_ProcPtrID_Create_EffectParameters = 0,		///< AAX_IEffectParameters creation procedure
	kAAX_ProcPtrID_Create_EffectGUI = 1,			///< AAX_IEffectGUI creation procedure
	kAAX_ProcPtrID_Create_HostProcessor = 3,		///< AAX_IHostProcessor creation procedure
	kAAX_ProcPtrID_Create_MixerParameters = 4,		///< AAX_IMixerParameters creation procedure, used by PIs that want ramped timestamp control changes
	kAAX_ProcPtrID_Create_EffectDirectData = 5		///< AAX_IEffectDirectData creation procedure, used by PIs that want direct access to their alg memory
};
...
/** @brief	A user-defined callback to initialize a private data block
 ...
 can use fewer functions that switch on @p iFieldIndex.
