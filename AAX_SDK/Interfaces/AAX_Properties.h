...
enum AAX_EProperty
{
     ...
     *  \li Apply this property at the \b ProcessProc level
     *
     *  \legacy \legacy For legacy plug-in session compatibility, this ID should match the Type ID
     *	used in the corresponding legacy TDM plug-in Types.
     */
    AAX_eProperty_PlugInID_TI = 15,
	/** \brief Four-character osid-style Effect identifier for dither native audio Effects
	 *
	 *	All registered product IDs (\ref AAX_eProperty_PlugInID_Native,
	 *	\ref AAX_eProperty_PlugInID_AudioSuite, and \ref AAX_eProperty_PlugInID_TI) must be
	 *	unique across all ProcessProcs registered within a single
     ...
     *
     *  \legacy For legacy plug-in session compatibility, this ID should match the Type ID used in
     *	the corresponding legacy Dither plug-in Types.
     */
    AAX_eProperty_PlugInID_Dither = 17,
	...
	/** \brief This property is not used by plug-ins based on the %AAX SDK
     */
    AAX_eProperty_PlugInID_Analysis = 20,
	...
	*/
	 ...
	 *  \ref AAX_eResourceType_PageTable. If \ref AAX_eProperty_StoreXMLPageTablesByType is not defined
	 *  or is set to 0 (the default) then the host will assume that all Effects in the collection use the same XML
