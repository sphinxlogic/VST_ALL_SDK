...
{
public:
	AAX_CPacket(AAX_CFieldIndex iFieldIndex) : mID(iFieldIndex), mDirty(true), mDataSize(0) {}
	~AAX_CPacket() {}
...
struct AAX_IPacketHandler
{
	...
	virtual AAX_Result			Call( AAX_CParamID iParamID, AAX_CPacket& ioPacket ) const = 0;
};
...
public:
	...
	AAX_Result Call( AAX_CParamID iParamID, AAX_CPacket& ioPacket ) const
	{
		if (fptEx)
			return (*pt2Object.*fptEx)( iParamID, ioPacket);
		else if (fpt)
