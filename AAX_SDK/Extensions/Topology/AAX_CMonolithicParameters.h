...
struct AAX_SInstrumentSetupInfo
{
	...
	//AAX Hybrid
	AAX_EStemFormat     mHybridInputStemFormat;     ///< Hybrid input stem format. A plug-in that defines this value must also define \c mHybridOutputStemFormat and implement \ref AAX_IEffectParameters::RenderAudio_Hybrid() \sa \ref additionalFeatures_Hybrid
	AAX_EStemFormat     mHybridOutputStemFormat;    ///< Hybrid output stem format. A plug-in that defines this value must also define \c mHybridInputStemFormat and implement \ref AAX_IEffectParameters::RenderAudio_Hybrid() \sa \ref additionalFeatures_Hybrid
	
	
	//General Properties
	AAX_EStemFormat	mInputStemFormat;               ///< Input stem format
	AAX_EStemFormat	mOutputStemFormat;              ///< Output stem format
    bool            mUseHostGeneratedGUI;           ///< Pro Tools or other host to generate a generic GUI for your plug-in?  (useful for early development)
	bool			mCanBypass;                     ///< Can this instrument be bypassed?
	AAX_CTypeID		mManufacturerID;                ///< Manufacturer ID
	AAX_CTypeID		mProductID;                     ///< Product ID
	AAX_CTypeID		mPluginID;                      ///< Plug-In (Type) ID
	AAX_CTypeID     mAudiosuiteID;                  ///< AudioSuite ID
	AAX_CBoolean    mMultiMonoSupport;              ///< MultiMono support
