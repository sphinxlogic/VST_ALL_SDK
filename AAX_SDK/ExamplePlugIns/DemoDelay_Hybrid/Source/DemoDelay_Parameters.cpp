...
AAX_Result DemoDelay_Parameters::EffectInit()
{
	...
	if (AAX_SUCCESS == result)
	{
		...
		result = Controller()->GetHybridSignalLatency(&hybridLatency);
	...
	if (AAX_SUCCESS == result)
	{
		...
		//Delay  (this won't go to the low latency algorithm, because we are doing this delay in the high latency algorithm.)
		...
		mPacketDispatcher.RegisterPacket(bypassID.CString(), AAX_FIELD_INDEX (SDemoDelayHybrid_LowLatency_Context, mCoefsDelayP), this, &DemoDelay_Parameters::GenerateCoefficientPacket);
