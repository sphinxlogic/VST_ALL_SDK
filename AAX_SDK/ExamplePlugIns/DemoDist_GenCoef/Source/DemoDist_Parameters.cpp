...
const float def_PI = 3.14159265358979323846264f;
...
{	
	// bypass
    mBypassID = cDefaultMasterBypassID;
	AAX_IParameter * masterBypass = new AAX_CParameter<bool>(
		mBypassID.CString(), AAX_CString("Master Bypass"), false,
	...
	// filter
	AAX_CParameter<float>* filtParameter = new AAX_CParameter<float>(
		DemoDist_FiltID, AAX_CString("Lowpass"), cDefaultParameterFilt,
		AAX_CLinearTaperDelegate<float>(0.0f, 1.0f),
		AAX_CNumberDisplayDelegate<float>(), true);
	filtParameter->SetNumberOfSteps(55);
	mParameterManager.AddParameter(filtParameter);
	
	// dist
	AAX_CParameter<float>* distParameter = new AAX_CParameter<float>(
		DemoDist_DistID, AAX_CString("Distortion"), cDefaultParameterDist,
		AAX_CLinearTaperDelegate<float>(0.0f, 2.0f),
		AAX_CNumberDisplayDelegate<float>(), true);
	distParameter->SetNumberOfSteps(55);
	mParameterManager.AddParameter(distParameter);
	
	// input gain
	AAX_CParameter<float>* inputGainParameter = new AAX_CParameter<float>(
	...
	inputGainParameter->SetNumberOfSteps(55);
	mParameterManager.AddParameter(inputGainParameter);

	// output gain
	AAX_CParameter<float>* outputGainParameter = new AAX_CParameter<float>(
		DemoDist_OutGID, AAX_CString("Output Gain"), cDefaultParameterOutG,
		AAX_CLogTaperDelegate<float>(0.002f, 2.0f),
		AAX_CDecibelDisplayDelegateDecorator<float>(AAX_CNumberDisplayDelegate<float>()), true);
	outputGainParameter->SetNumberOfSteps(55);
	mParameterManager.AddParameter(outputGainParameter);
...
AAX_Result DemoDist_Parameters::UpdateParameterNormalizedValue(AAX_CParamID iParamID, double aValue, AAX_EUpdateSource inSource)
{
	...
	if (strcmp(iParamID, mBypassID.CString()) == 0)	
	{
		mDirtyBypassFlag = true;
	}
	else if (strcmp(iParamID, DemoDist_MixID) == 0)	
	{
		mDirtyMixFlag = true;
	}
	else if (strcmp(iParamID, DemoDist_FiltID) == 0)	
	{
		mDirtyFiltFlag = true;
	}
	else if (strcmp(iParamID, DemoDist_DistID) == 0)	
	{
		mDirtyDistFlag = true;
	}
	else if (strcmp(iParamID, DemoDist_InpGID) == 0)	
	{
		mDirtyInpGainFlag = true;
	}
	else if (strcmp(iParamID, DemoDist_OutGID) == 0)	
	{
		mDirtyOutGainFlag = true;
	}
	
	// Keep track of changes for the Compare Light	
	mNumPlugInChanges++;
...
AAX_Result DemoDist_Parameters::GenerateCoefficients()
{
	...
	{
		AAX_IParameter* parameter = mParameterManager.GetParameterByID(mBypassID.CString());
		if (parameter)
		{
			bool theBypassValue;
