...
AAX_Result DemoSynth_Parameters::EffectInit()
{
	...
	for (int32_t n = 0; n < kNumInputMIDINodes; ++n)
	{
		...
		{
			AAX_IParameter* const param = new AAX_CParameter<int32_t>(
				GetNodeID(n),
				GetNodeNameString(n),
	...
	if ((ioPacketsRemaining > 0) && (NULL != ioPacket))
	{
		...
		if (ioPacket->mTimestamp <= inSample)
		{
			switch (ioPacket->mData[0] & 0xF0)
			{
				...
				case kMIDIChannelMode:
				{
					if (kMIDIChannelMode_AllNotesOffCmd == ioPacket->mData[1])
					{
...
{
	static std::vector<AAX_IString*> sNodeAssignmentStateStrings(eNodeAssignment_NumStates, new AAX_CString());
	
	if (0 == sNodeAssignmentStateStrings[0]->Length())
	{
		...
		for (int32_t i = eNodeAssignment_SingleStemBegin; i < eNodeAssignment_SingleStemEnd; ++i)
		{
			std::stringstream ss;
