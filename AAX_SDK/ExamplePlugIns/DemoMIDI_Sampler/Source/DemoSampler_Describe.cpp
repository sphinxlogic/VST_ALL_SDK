...
AAX_CEffectID kEffectID_DemoSampler	= "com.avid.aax.sdk.demosampler";
...
AAX_Result GetEffectDescriptions( AAX_ICollection * outCollection )
{
	...
	err |= AAX_CMonolithicParameters::StaticDescribe(effectDescriptor, setupInfo);			AAX_ASSERT (err == AAX_SUCCESS);
	
	//
	// Add custom properties
	AAX_IPropertyMap* properties = effectDescriptor->NewPropertyMap();
	if (properties)
	{
		properties->AddProperty(AAX_eProperty_SampleRate, AAX_eSampleRateMask_44100); // Samples are only set up for playback at 44.1 kHz
		properties->AddProperty(AAX_eProperty_UsesClientGUI, true); // Use the host-generated GUI
		...
		effectDescriptor->SetProperties(properties);
	}
	...
	if ( err == AAX_SUCCESS )
	{
		err = outCollection->AddEffect( kEffectID_DemoSampler, effectDescriptor );			AAX_ASSERT (err == AAX_SUCCESS);
	}
	
	outCollection->SetManufacturerName( "Avid" );
	outCollection->AddPackageName( "DemoMIDISampler Plug-In" );
	outCollection->AddPackageName( "DemoSampler" );
	outCollection->AddPackageName( "DmSY" );		
