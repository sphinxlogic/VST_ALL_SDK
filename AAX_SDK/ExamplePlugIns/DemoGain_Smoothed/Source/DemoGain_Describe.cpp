...
AAX_CEffectID kEffectID_DemoGain	= "com.avid.aax.sdk.demogain.smoothed";
...
static void DescribeAlgorithmComponent( AAX_IComponentDescriptor * outDesc )
{
	...
	err = outDesc->AddSampleRate (eAlgFieldID_SampleRate);            AAX_ASSERT (err == AAX_SUCCESS);
	...
	err = outDesc->AddDataInPort ( eAlgPortID_TargetCoefsGainIn, sizeof (SDemoGain_CoefsGain) );  AAX_ASSERT (err == AAX_SUCCESS);
	//
	// Register private data fields
	err = outDesc->AddPrivateData( eAlgPortID_CurCoefsGain, sizeof (SDemoGain_CoefsGain), AAX_ePrivateDataOptions_DefaultOptions);	AAX_ASSERT (err == AAX_SUCCESS);
...
#ifndef _DEBUG // Debug builds do not report cycles - 1 instance per chip
	properties->AddProperty ( AAX_eProperty_TI_InstanceCycleCount, 140 );
...
static AAX_Result DemoGain_GetPlugInDescription( AAX_IEffectDescriptor * outDescriptor )
{
	...
	// Effect identifiers
	//
	outDescriptor->AddName ( "Demo Gain (Smoothed)" );
