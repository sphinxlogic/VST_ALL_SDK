...
static AAX_Result Delay_GetPlugInDescription( AAX_IEffectDescriptor * outDescriptor )
{
	...
	// Effect identifiers
	//
	outDescriptor->AddName ( "Demo Delay (HostProcessor)" );
	outDescriptor->AddName ( "Demo Delay (HP)" );
	outDescriptor->AddName ( "DmDly HP" );
	outDescriptor->AddName ( "DmDly" );
	outDescriptor->AddName ( "Dly" );
    ...
    err = outDescriptor->AddProcPtr((void *) DemoDelay_HostProcessor::Create, kAAX_ProcPtrID_Create_HostProcessor);
...
AAX_Result HostProcessor_GetEffectDescriptions( AAX_ICollection * outCollection )
{
	...
	const AAX_CEffectID		effectID_DemoDelay	= "com.avid.aax.sdk.demodelay.hostprocessor";
	...
	if ( plugInDescriptor )
	{
		...
		if ( result == AAX_SUCCESS )
			result = outCollection->AddEffect( effectID_DemoDelay, plugInDescriptor );
